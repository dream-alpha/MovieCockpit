...
class Actions():
	...
	def initColorActions(self, csel):
		...
		self.color_buttons_matrix = [
			[												  # level 0
				...
				[_("Home"), boundFunction(csel.changeDir, MountCockpit.getInstance().getHomeDir(ID))]		  # blue
			],
			[												  # level 1
				[_("Delete"), csel.deleteMovies],								  # red
	...
	def setTrashcanActions(self, csel, path):
		...
		if path and "trashcan" in path:
			...
			self.color_buttons_matrix[0][KEY_BLUE] = [_("Home"), boundFunction(csel.changeDir, MountCockpit.getInstance().getHomeDir(ID))]
		else:
