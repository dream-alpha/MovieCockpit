	...
	def __init__(self, session, csel, menu_mode, current_dir, list_styles, service=None):
		...
		if self.menu_mode == MENU_FUNCTIONS:
			...
			if self.current_dir not in MountCockpit.getInstance().getMountedBookmarks(ID):
				menu.append((_("Home"), (boundFunction(self.csel.changeDir, MountCockpit.getInstance().getHomeDir(ID)), True)))
				menu.append((_("Directory up"), (boundFunction(self.csel.changeDir, self.current_dir + "/.."), True)))

			menu.append((_("Toggle show deleted movies"), (self.csel.toggleShowDeletedMovies, True)))
			...
			for i, _list_style in enumerate(self.list_styles):
				menu.append((_(self.list_styles[i][1]), (boundFunction(self.csel.movie_list.setListStyle, i), True)))
			...
			menu.append((_("Remove all marks"), (self.csel.removeCutListMarkers, True)))
			menu.append((_("Setup"), (self.csel.openConfigScreen, True)))
			menu.append((_("About"), (boundFunction(about, self.session), True)))
