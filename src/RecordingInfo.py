...
class RecordingInfo(Screen, HelpableScreen):
	...
	def fillList(self):
		...
		alist.append(("SQL: Event start", str(datetime.fromtimestamp(self.afile[FILE_IDX_EVENT_START_TIME]))))
		alist.append(("SQL: Recording start", str(datetime.fromtimestamp(self.afile[FILE_IDX_RECORDING_START_TIME]))))
		alist.append(("SQL: Recording stop", str(datetime.fromtimestamp(self.afile[FILE_IDX_RECORDING_STOP_TIME]))))
		alist.append(("", ""))
		alist.append(("EIT: File exists", str(os.path.exists(os.path.splitext(path)[0] + ".eit"))))
		if eit:
			alist.append(("EIT: Event Start", str(datetime.fromtimestamp(eit["start"]))))
			alist.append(("EIT: Length", str(eit["length"] / 60)))
		alist.append(("", ""))
		alist.append(("META: File exists", str(os.path.exists(path + ".meta"))))
		if meta:
			alist.append(("META: Rec time", str(datetime.fromtimestamp(meta["rec_time"]))))
		alist.append(("", ""))
		alist.append(("XMETA: File exists", str(os.path.exists(path + ".xmeta"))))
		if meta:
			alist.append(("XMETA: Recording start", str(datetime.fromtimestamp(meta["recording_start_time"]))))
			alist.append(("XMETA: Recording stop", str(datetime.fromtimestamp(meta["recording_stop_time"]))))
