...
from .FileManagerUtils import FILE_TYPE_FILE, FILE_IDX_TYPE, FILE_IDX_NAME, FILE_IDX_EVENT_START_TIME,\
...
class Info():
	...
	def getBeginTimeString(self):
		...
		if event_start_time:
			...
			stime = datetime.fromtimestamp(event_start_time).strftime(movie_date_format)
		return stime
	...
	def getSize(self):
		...
		if self.afile and self.afile[FILE_IDX_TYPE] == FILE_TYPE_FILE:
			...
			_count, size = FileManager.getInstance().getMovieCountSize(self.path)
		return size
	...
	def getCover(self):
		...
		if self.afile:
			...
			bfile = FileManager.getInstance().getMovieFile("covers", os.path.basename(path))
			if bfile:
			...
			if not cover:
				...
				bfile = FileManager.getInstance().getMovieFile("covers", adir_name)
				if bfile:
					cover = bfile[COVER_IDX_COVER]
